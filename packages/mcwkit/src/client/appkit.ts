...
import { ErrorUtil, HelpersUtil, ConstantsUtil as UtilConstantsUtil } from '@mcw/mcwkit-utils'
import { W3mFrameHelpers, W3mFrameProvider } from '@mcw/mcwkit-wallet'
import type { W3mFrameTypes } from '@mcw/mcwkit-wallet'
import { W3mFrameRpcConstants } from '@mcw/mcwkit-wallet/utils'
    ...
    if (!isInitialized && CoreHelperUtil.isClient()) {
      ...
      if (features) {
        ...
        if (usingEmbeddedWallet) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/embedded-wallet'))
        }

        if (features.email) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/email'))
        }
        if (features.socials) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/socials'))
        }

        if (features.swaps) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/swaps'))
        }

        if (features.send) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/send'))
        }

        if (features.receive) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/receive'))
        }

        if (features.onramp) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/onramp'))
        }

        if (features.history) {
          featureImportPromises.push(import('@mcw/mcwkit-scaffold-ui/transactions'))
        }
      }

      await Promise.all([
        ...featureImportPromises,
        import('@mcw/mcwkit-scaffold-ui'),
        import('@mcw/mcwkit-scaffold-ui/w3m-modal')
      ])
